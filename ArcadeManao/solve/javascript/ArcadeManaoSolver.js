...
function main() {
  try {
    ...
    var reader = new tc.Reader(input);
        var level = reader.next('String[]'); reader.next();
    var coinRow = reader.next('int'); reader.next();
    var coinColumn = reader.next('int');

    var ArcadeManao = require("./ArcadeManao");
    var result = ArcadeManao.shortestLadder(level, coinRow, coinColumn);
