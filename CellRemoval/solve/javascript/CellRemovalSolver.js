...
function main() {
  try {
    ...
    var reader = new tc.Reader(input);
        var parent = reader.next('int[]'); reader.next();
    var deletedCell = reader.next('int');

    var CellRemoval = require("./CellRemoval");
    var result = CellRemoval.cellsLeft(parent, deletedCell);
